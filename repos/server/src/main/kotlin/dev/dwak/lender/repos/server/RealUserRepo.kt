...
class RealUserRepo(
    ...
    override suspend fun tokenExists(token: String): Boolean = tokenQueries.tokenExists(token).executeAsOne()

    override suspend fun getUserByToken(token: String): ServerUser {
        return tokenQueries.getUserByToken(token) { id, email, created_at ->
            ServerUser(
                id = id,
                email = email,
    ...
    override suspend fun getUserById(id: String): ServerUser = userQueries
        .getById(id) { id, email, password, created_at ->
            ServerUser(
                id = id,
                email = email,
                createdAt = Instant.parse(created_at)
